...
// une seule ecoute deleguee sur le conteneur, plutot que 3 ecoutes
// separees : plus robuste, ca evite tout risque qu'un des 3 boutons se
// retrouve orphelin de son ecouteur
document.querySelector(".gifts-row").addEventListener("click", (e) => {
  const box = e.target.closest(".gift-box");
  if(!box) return;
  const which = box.dataset.gift;
  playClickSound();
  if(which === "souvenirs"){
    showScreen("screen-souvenirs");
    albumGoTo(0, true);
  } else if(which === "histoire"){
    showScreen("screen-histoire");
    histoireGoTo(0);
  } else if(which === "surprise"){
